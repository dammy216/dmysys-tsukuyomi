 ...
 * INTRO2_BLINK_START_SECONDS から始まる ―― それより前は intro2Blink=1 の
 * 常時点灯のまま、可動(スイープ・スナップ)だけが先に動いている状態になる。
 * 当初はカメラの引きが終わるタイミング(11.8秒)にすぐ点滅させていたが、
 * 「動くだけで点滅しない時間が短すぎる」というユーザー指摘で
 * breath開始(23秒)の手前・21.7秒まで伸ばした ―― 点滅が乗るまでの間、
 * スイープ・スナップの動きだけをしっかり見せる時間を長く取っている。
 ...
 * もとは dronePathData の引きの着地点(t:11.5 で radius 11.5→53、そこから
 * 詰め始める)あたりの11.8に合わせていたが、「可動だけが先に動く」区間が
 * 短すぎるというユーザー指摘で、21.7秒あたり(breath開始23秒の手前)まで
 * 延長した ―― 点滅が始まるまでの、動くだけで点滅しない時間を長く取ることで
 * 上下スイープ+左右スナップの動きそのものをしっかり見せる。
 */
const INTRO2_BLINK_START_SECONDS = 21.7;
  ...
  useFrame(({ clock }, delta) => {
    ...
    */
    ...
    /*
      ...
      計算)。ここで求めるのは**点滅係数だけ** ―― INTRO2_BLINK_START_SECONDS
      (21.7秒。定数のコメント参照)まではブリンクさせず常時点灯、そこから
      拍のON/OFFに切り替える。
    */
