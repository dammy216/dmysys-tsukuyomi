...
/*
  Reply演出中、灯籠は水面を離れて空へ昇っていく(gatherRef で駆動)。

  **天守のまわりへ寄せ集めない。** 各灯籠は元の(x, z)位置に立ったまま、
  高さだけ GATHER_HEIGHT_MIN〜MAX の範囲へ上がる(gatherHeightT で個体ごとに
  振り分け)。水面いっぱいに散らばった灯籠がそのまま一斉に空へ上がっていく、
  灯篭上げ(灯籠流しの逆)のような画にする指定。
*/
/** 昇っていく間の漂い(円軌道の半径)。まっすぐ一直線に上がると機械的に見える */
const GATHER_DRIFT_RADIUS = 1.2;
/**
 * 昇る速さに個体差を付ける幅(gatherRef の値換算)。0だと全灯籠が
 * 完全に同時に動いてしまい、何本もの光が違うタイミングで空へ昇っていく
 * 感じが出ない。
 */
const GATHER_DELAY_SPREAD = 0.4;
/** 昇りきったときの明るさの上乗せ(1.0で+100%)。空高くのぼるほど白熱させる */
...
type LanternsProps = {
   ...
   * 元の(x, z)位置に立ったまま GATHER_HEIGHT_MIN〜MAX の高さまで昇りきる。
   * 星降る海の activationRef と同じ理由で数値 prop ではなく ref
   * (毎フレーム変わる値なので、state だと灯籠900個ぶんの再レンダーが走る)。
   * 省略時は常に水面のまま。
   */
  gatherRef?: RefObject<number>;
  /** 昇る高さの下限。水面すれすれを渡す想定 */
  gatherHeightMin?: number;
  /** 昇る高さの上限。塔の頂部より上あたりを渡す想定 */
  gatherHeightMax?: number;
  /**
   * カメラが引いたあと、昇る高さをさらに広げる進み具合(0〜1)を持つ ref。
   * 11秒直後の高さ(gatherHeightMin/Max)のままだと、そのあとカメラが
   * 大きく引くカット(ReplyCamera の PATH)では低い位置に収まってしまう。
   * この値を上げると expandHeight の広い範囲へブレンドし、引いた画でも
   * 画面いっぱいに灯籠がある見た目を保つ。省略時は常に gatherHeight のまま
   * (広げない)。
   */
 ...
 *
 * Reply演出中は gatherRef で、元の(x, z)位置に立ったまま空へ昇っていき
 * (寄せ集めない見た目にする指定。gatherHeightMin/Max のコメント参照)、
 * さらに expandRef でカメラが引いたぶん昇る高さをより大きく広げる
 * (expandHeightMin/Max のコメント参照)。
 */
export function Lanterns({
  ...
  */
  ...
  useFrame(({ clock }) => {
    ...
    /*
      カメラが引いたぶん、昇る高さを expandHeight の広い範囲へブレンドする。
      1周ぶんの計算なので毎フレームここで求めて全灯籠で使い回す
      (per-lantern の値ではない)。

      expandRef は0→1の等速な一次ランプで、1に達した
    ...
    */
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      /*
        ...
        (=速く動く)、何本もの光が違うタイミングで空へ昇っていく感じになる。
      */
      ...
      if (eased > 0) {
        /*
          元の(x, z)位置に立ったまま高さだけ昇る(このファイル冒頭のコメント
          参照。天守のまわりへ寄せ集めない)。昇る高さは個体ごとに
          gatherHeightT で GATHER_HEIGHT_MIN〜MAX へ振り分ける。
        */
        const targetY = heightMin + (heightMax - heightMin) * data.gatherHeightT;

        // まっすぐ一直線に昇ると機械的に見えるので、ゆっくり円軌道で漂わせる
        const driftAngle = elapsed * data.speed * 0.6 + data.phase;
        const drift = eased * GATHER_DRIFT_RADIUS;

        px += Math.cos(driftAngle) * drift;
        pz += Math.sin(driftAngle) * drift;
