...
- **タイムライン/キーフレームアニメーション(カメラワーク等)は Theatre.js を使わない**
  (導入したが撤去済み)。各featureのカメラコンポーネント内にキーフレーム配列を直書きし、
  `songTime`/経過時間で直接補間する。既存の実装例は `features/reply/ReplyCamera.tsx`
  (`DRONE_PATH`/`sampleDrone`)と `features/starfall-sea/StarfallCamera.tsx`
  (`PATH`/`samplePath`)参照。
